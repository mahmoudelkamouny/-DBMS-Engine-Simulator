...
public class RTreeInnerNode<T extends Comparable<T>> extends RTreeNode<T>  implements Serializable
{
	 ...
	 */
	public RPushUp<T> insert(T key, Ref recordReference, RTreeInnerNode<T> parent, int ptr,String col_name,String table_name)
	{
		int index = findIndex(key);
		RPushUp<T> RPushUp = children[index].insert(key, recordReference, this, index, col_name,table_name);
		...
		if(this.isFull())
		{
			RTreeInnerNode<T> newNode = this.split(RPushUp);
		...
		else
		{
			...
			while (index < numberOfKeys && getKey(index).compareTo(key) < 0)
				++index;
			this.insertRightAt(index, RPushUp.key, RPushUp.newNode);
			return null;
		}
	}
	/**
	 * split the inner node and adjust values and pointers.
	 * @param RPushUp key to be pushed up to the parent in case of splitting.
	 * @return Inner node after splitting
	 */
	@SuppressWarnings("unchecked")
	public RTreeInnerNode<T> split(RPushUp<T> RPushUp) 
	{
		int keyIndex = this.findIndex((T)RPushUp.key);
		...
		if(keyIndex < totalKeys / 2)
			this.insertRightAt(keyIndex, RPushUp.key, RPushUp.newNode);
		else
			newNode.insertRightAt(keyIndex - midIndex, RPushUp.key, RPushUp.newNode);
	...
	{
		return children[findIndex(key)].search(key,page, page_index);
	}
	...
	public Ref get(T key,String table_name,String col_name,Hashtable<String, Object> htblColNameValue) {
		return children[findIndex(key)].get(key,table_name,col_name, htblColNameValue);
	}
	...
	public boolean update(T key,Ref old_reference,Ref new_reference,int ptr,RTreeInnerNode<T> parent,boolean flag){
		boolean flag2=false;
		for (int i=0;i<numberOfKeys;++i) {
			if (keys[i].compareTo(key) >= 0){
				flag2=children[i].update(key,old_reference,new_reference,i,this,flag);
				if(flag2==true){
					return flag2;
				}
					}
		...
		if (flag2==false){
			flag2 = children[numberOfKeys].update(key,old_reference,new_reference,numberOfKeys,this,flag);}
		return flag2;
	}
	

	public boolean update_for_insert(T key,Ref old_reference,Ref new_reference,int ptr,RTreeInnerNode<T> parent){
		boolean flag2=false;
		for (int i=0;i<numberOfKeys;++i) {
			if (keys[i].compareTo(key) >= 0){
				flag2=children[i].update_for_insert(key,old_reference,new_reference,i,this);
				if(flag2==true){
					return flag2;
				}
					}
		...
		if (flag2==false){
			flag2 = children[numberOfKeys].update_for_insert(key,old_reference,new_reference,numberOfKeys,this);}
		return flag2;
	}
	public boolean delete_using_Ref(T key, Ref r, RTreeInnerNode<T> parent, int ptr) {
		boolean done = false;
		for (int i = 0; !done && i < numberOfKeys; ++i) {
			System.out.println(key);
			if (keys[i].compareTo(key) >= 0)
				done = children[i].delete_using_Ref(key, r, this, i);
          
		}
		if (!done)
			done = children[numberOfKeys].delete_using_Ref(key, r, this, numberOfKeys);
		if (numberOfKeys < this.minKeys()) {
			if (isRoot()) {
				this.getFirstChild().setRoot(true);
				this.setRoot(false);
